...
for(i in 1:n_sims){
  ...
  order_res <- mcer::order_select(mcchain, 6, F)
  bic_order <- which(order_res[,3] == min(order_res[,3]))
  aic_order <- which(order_res[,2] == min(order_res[,2]))
  ent_aic <- mcer::efficient_mc_er(mcchain, aic_order)
  ent_bic <- mcer::efficient_mc_er(mcchain, bic_order)

  results_mat_sim1[i, ] <- c(ent1$entropy_rate,
                             ent2$entropy_rate,
                             ent2$entropy_rate,
                             ent4,
...
set.seed(124)
n_sims <- 1000
results_mat_sim2 <- matrix(NA, nrow=n_sims, ncol=6)
state_space <- c('1', '2')
order_mat2 <- matrix(NA, nrow = n_sims, ncol=2)
for(i in 1:n_sims){
    ...
    results_mat_sim2[i, ] <- c(ent1$entropy_rate,
                               ent2$entropy_rate,
                               ent2$entropy_rate,
                               ent4,
                               ent_aic$entropy_rate,
                               ent_bic$entropy_rate)[6:1]
    order_mat2[i,] <- c(aic_order, bic_order)
}
colnames(results_mat_sim2) <- c('m=1', 'm=2', 'm=3', 'SWLZ', 'AIC', 'BIC')[6:1]
boxplot(results_mat_sim2, horizontal=T, ylim=c(0.25, 1))
abline(v=true_ent_sim2)
apply(order_mat2, 2, table)
#-------------------------------------------------------------------------------
# Simulation 3 -----------------------------------------------------------------
p = 0.4
a = 0.01
...
set.seed(124)
n_sims <- 1000
results_mat_sim3 <- matrix(NA, nrow=n_sims, ncol=6)
state_space <- c('1', '2')
order_mat3 <- matrix(NA, nrow = n_sims, ncol=2)
for(i in 1:n_sims){
    ...
    results_mat_sim3[i, ] <- c(ent1$entropy_rate,
                               ent2$entropy_rate,
                               ent2$entropy_rate,
                               ent4,
                               ent_aic$entropy_rate,
                               ent_bic$entropy_rate)[6:1]
    order_mat3[i,] <- c(aic_order, bic_order)
}
colnames(results_mat_sim3) <- c('m=1', 'm=2', 'm=3', 'SWLZ', 'AIC', 'BIC')[6:1]
