...
// [[Rcpp::export]]
Rcpp::List mcer(std::string mc_seq,
            int mc_order,
    ...
    tm = tc / rs;
    arma::mat ll = tc % arma::log(tm);
    arma::mat res = - tm % log2(tm) % sm_mat;

    res.replace(arma::datum::nan, 0);
    ll.replace(arma::datum::nan, 0);

    // return arma::accu(res);
    return Rcpp::List::create(
        _["log_likelihood"] = arma::accu(ll),
        _["transition_matrix"] = tm,
        _["stationary_matrix"] = sm,
        _["entropy_rate"] = arma::accu(res)
    );
}
